...
Description:
This is a demonstration of detecting the minimum translation vector of two circles in 2D.
There are Pink and Yellow wireframes of circles, when the circles overlap, the MVP will be
drawn over the overlap distance indicating the intersection direction & magnitude between the two circles.

The algorithm used is a modified version of the Separating Axis Theorem which is made much faster and more
trivial because of both objects being circles. The only possible axis for intersection is
the vector from the center of one circle to the other. Overlap over this axis is tested,
if overlap is found, the axis is normalized to get the minimum translation vector. the amount of overlap
is saved as the magnitude of the intersection.
...
{	
	...
	//Check for collision
	collision = CheckCollision(*circle1Collider, *circle2Collider, minimumTranslationVector, overlap);
...
void renderScene()
{
	...
	if(collision)
	{
		glm::mat4 mvp = VP * circle1->GetModelMatrix();

		glUseProgram(0);
		glLineWidth(10.0f);

		//Draw the MTV between them
		glColor3f(1.0f, 1.0f, 1.0f);

		//Get the MTV's endpoints in world space
		glm::vec4 startPoint = glm::vec4(glm::vec2(circle1Collider->center), 0.11f, 1.0f) - glm::vec4(circle1Collider->radius * minimumTranslationVector, 0.0f, 0.0f);
		glm::vec4 endPoint = glm::vec4(glm::vec2(circle1Collider->center), 0.11f, 1.0f) - glm::vec4(circle1Collider->radius * minimumTranslationVector, 0.0f, 0.0f) + glm::vec4(overlap * minimumTranslationVector, 0.0f, 0.0f);
		startPoint = VP * startPoint;
...
void main()
{
	...
	// Create a window
	window = glfwCreateWindow(800, 800, "Detecting the Minimum Translation Vector (Circle - 2D)", nullptr, nullptr);
